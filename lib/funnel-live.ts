/**
 * Live funnel provider — an Attio pipeline mapped into the same FunnelJourney
 * shape the seeded funnel uses, so /funnel swaps sources at the repo level
 * exactly as designed. Honest by construction: returns null when the key is
 * missing or Attio is unreachable (the page falls back to seed and says so),
 * Closed Lost is excluded but counted, unknown stages are skipped.
 *
 * ICP fit is a transparent heuristic over the qualification fields Attio
 * carries (budget / pain points / timeline / description / deal value) — it
