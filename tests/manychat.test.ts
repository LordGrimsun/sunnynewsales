...
describe('parseManyChatPageInfo', () => {
  it('reads name / username / pro from data', () => {
    expect(
      parseManyChatPageInfo({ status: 'success', data: { name: 'the operator', username: 'vantage.os', is_pro: true } }),
    ).toEqual({ name: 'the operator', username: 'vantage.os', isPro: true });
  });

  it('tolerates a missing username and non-pro accounts', () => {
    expect(parseManyChatPageInfo({ data: { name: 'the operator' } })).toEqual({
      name: 'the operator',
      username: null,
      isPro: false,
    });
...
describe('manychatStatus', () => {
  ...
  it('is connected when getInfo returns a page, showing the handle', async () => {
    const fetchOk = (async () =>
      ({ ok: true, status: 200, json: async () => ({ data: { name: 'the operator', username: 'vantage.os', is_pro: true } }) }) as unknown as Response) as typeof fetch;
    const s = await manychatStatus({ MANYCHAT_API_KEY: 'sk-test' }, fetchOk);
    expect(s.state).toBe('connected');
    expect(s.detail).toMatch(/@vantage\.os/);
  });
